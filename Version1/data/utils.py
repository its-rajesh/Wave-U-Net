...
def random_amplify(mix, targets, shapes, min, max):
    ...
    for key in targets.keys():  
        if  key != "mix":
            #print("Key=", key)
            #print('--------', residual, '--------', targets[key], sep='\n')
...
def load(path, sr=22050, mono=True, mode="numpy", offset=0.0, duration=None):
    y, curr_sr = librosa.load(path, sr=sr, mono=True, res_type='kaiser_fast', offset=offset, duration=duration) #mono=True Fixed written
