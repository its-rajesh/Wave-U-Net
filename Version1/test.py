...
def evaluate(args, dataset, model, instruments):
    ...
    with torch.no_grad():
        for example in dataset:
            print("Evaluating " + example["mixed"])
            ...
            pred_sources = predict_song(args, example["mixed"], model)
            pred_sources = np.stack([pred_sources[key].T for key in instruments])

            # EDITED
            if target_sources.shape[1] > pred_sources.shape[1]:
                target_sources = target_sources[:, :pred_sources.shape[1]]
            else:
                pred_sources = pred_sources[:, :target_sources.shape[1]]
