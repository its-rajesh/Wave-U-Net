...
class SeparationDataset(Dataset):
    def __init__(self, dataset, partition, instruments, sr, channels, shapes, random_hops, hdf_dir, audio_transform=None, in_memory=False):
        ...
        instruments = ['mridangam_left', 'mridangam_right', 'violin', 'vocal'] #Edited
        ...
        if not os.path.exists(self.hdf_dir):
            ...
            with h5py.File(self.hdf_dir, "w") as f:
                ...
                for idx, example in enumerate(tqdm(dataset[partition])):
                    # Load mix
                    #print(example)
                    mix_audio, _ = load(example["mixed"], sr=self.sr, mono=(self.channels == 1))
                    ...
                    #print('-----------------------',source_audios.shape, mix_audio.shape)
                    #print(source_audios.shape, mix_audio.shape)
                    #print(source_audios)
                    #print(mix_audio)
                    
                    # EDITED
                    if source_audios.shape[1] > mix_audio.shape[1]:
                        source_audios = source_audios[:, :mix_audio.shape[1]]
                    else:
                    	mix_audio = mix_audio[:, :source_audios.shape[1]]
        ...
        #print('-----STARTPOS-----:', self.start_pos)
        try: #Edited
            self.length = self.start_pos[-1] #Actual
        except:
        	self.length = len(self.start_pos)
    ...
    def __getitem__(self, index):
        ...
        if hasattr(self, "audio_transform") and self.audio_transform is not None:
            #print('---------', audio, audio.shape, '--------', targets['mridangam_left'], targets['mridangam_left'].shape, sep='\n')
