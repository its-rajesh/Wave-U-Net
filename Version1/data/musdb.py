...
def get_musdbhq(database_path):
    ...
    for subset in ["train", "test"]:
        ...
        for track_folder in sorted(tracks):
            ...
            example = dict()
            for stem in ["mixed", "mridangam_left", "mridangam_right", "violin", "vocal"]:
                filename = stem if stem != "mix" else "mixture" #mix and mixture
            ...
            if not os.path.exists(acc_path):
                ...
                stem_audio = []
                for stem in ["mridangam_left", "mridangam_right", "violin"]:
    ...
    for subset in ["train", "test"]:
        ...
        for track in sorted(tracks):
            ...
            if os.path.exists(mix_path):
                ...
                paths.update({key : track_path + "_" + key + ".wav" for key in ["mridangam_left", "mridangam_right", "violin", "vocal"]})
            ...
            paths = dict()
            stem_audio = dict()
            for stem in ["mridangam_left", "mridangam_right", "violin", "vocal"]:
            ...
            # Add other instruments to form accompaniment
            acc_audio = np.clip(sum([stem_audio[key] for key in list(stem_audio.keys()) if key != "vocal"]), -1.0, 1.0)
            ...
            diff_signal = np.abs(mix_audio - acc_audio - stem_audio["vocal"])
...
def get_musdb_folds(root_path, version="HQ"): #version="HQ"
    if version == "HQ":
    ...
    train_list = np.random.choice(train_val_list, 65, replace=False)
